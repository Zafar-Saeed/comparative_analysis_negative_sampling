...
def read_dataset(path, results_dir,dataset_name, dev_mode=True,max_examples = float('inf')):
    index = Index()
    ...
    data_set['train'] = read_file(os.path.join(dataset_path,'train'),index,max_examples)
    if dev_mode:
        # data_set['test'] = read_file(os.path.join(path,'dev'),index,max_examples)
        data_set['test'] = read_file(os.path.join(dataset_path,'dev'),index,max_examples)
    else:
        # data_set['test'] = read_file(os.path.join(path, 'test'),index, max_examples)
        data_set['test'] = read_file(os.path.join(dataset_path, 'test'),index, max_examples)
    
    # data_set['dev'] = read_file(os.path.join(path, 'dev'), index, max_examples)
    data_set['dev'] = read_file(os.path.join(dataset_path, 'dev'), index, max_examples)
    data_set['num_ents'] = index.ent_vocab_size()
    data_set['num_rels'] = index.rel_vocab_size()
    # index.save_index(results_dir)
    index.save_index(dataset_path)
    # ************************************************************
