...
class Negative_Sampler(object):
    def __init__(self,triples,num_samples,filtered):
        ...
        # Zafar Edit: 30-Oct-2024 - Temporarily adding this code to get the statistics about how many entities have true candidates for the num_of_negs after filiteration
        self.filtered_candidates_dict = dict()
        self.filtered_candidates_dict["targets"] = dict()
        self.filtered_candidates_dict["sources"] = dict()
...
class Static_Sampler(Negative_Sampler):
    ...
    def sample(self,ex,is_target,candidates,num_samples=0):
        ...
        # Zafar Edit: 30-Oct-2024 - for recording number of possible samples 
        # following code might add substantial overhead. Run the code only for one epoch to get the final true negative candidates for each entity
        if is_target:
            key_type = "targets"
            ent = ex.s   
        else:
        ...
        entities_with_candidate_list = self.filtered_candidates_dict.get(key_type, dict())
        ent_candidates = entities_with_candidate_list.get(ent, set())
        if len(ent_candidates) > 0:
            dummy = 0
        entities_with_candidate_list[ent] = ent_candidates.union(samples)                
        # *************************************************************
...
class Dynamic_Sampler(Negative_Sampler):
    ...
    def load_rescal(self, dataset_path):
        # from models import Rescal
        from negateive_samplers.kge_rl.models import Rescal
        import torch

        # Zafar Edit: 22-Oct-2024. building pre-trained rescal model based on the dataset **********************
        # path = "/home/mitarb/kotnis/Data/neg_sampling/wordnet/corrupt/rescal/rescal_100/rescal_params.pt"
        # TODO: add more conditions for new datasets
        dataset_name = os.path.basename(dataset_path)
        if dataset_name.lower() == "fb15k":
            num_of_ents = constants.fb15k_ents
            num_of_rels = constants.fb15k_rels
        elif dataset_name.lower() == "wn18":
            num_of_ents = constants.wn_ents
            num_of_rels = constants.wn_rels
        elif dataset_name.lower() == "kinships":
            num_of_ents = constants.kinships_ents
            num_of_rels = constants.kinships_rels
        else: # TODO: NEED TO ADD THIS INFO. FOR ALL THE DATASETS
            print("Number of entities and relationships are not declared in constant.py file")

        # model =  Rescal(constants.fb15k_ents,constants.fb15k_rels,100)
        # TODO: Need to add a parameter to load different types of Generators other than Rescal
        model =  Rescal(num_of_ents,num_of_rels,100)
        # state_dict = torch.load(path)

        model_param_file = "rescal_params.pt"

        state_dict = torch.load(os.path.join(dataset_path,model_param_file))
        # state_dict = torch.load(os.path.join(dataset_path,"rescal_100.model")) # For testing purpose only
        # *********************************************************************************************************

        # model.entities.weight.data = state_dict['entity_representations.0._embeddings.weight']
        # model.rels.weight.data = state_dict['relation_representations.0._embeddings.weight']
        
        if model_param_file.endswith(".model"): # loading models that are trained using Pykeen, otherwise, it loads models trained by experiment_runner.py
            state_dict['entities.weight'] = state_dict.pop('entity_representations.0._embeddings.weight')
            state_dict['rels.weight'] = state_dict.pop('relation_representations.0._embeddings.weight')

            state_dict.pop('entity_representations.0.regularizer.weight')
            state_dict.pop('entity_representations.0.regularizer.regularization_term')
            state_dict.pop('relation_representations.0.regularizer.weight')
            state_dict.pop('relation_representations.0.regularizer.regularization_term')
...
class NN_Sampler(Dynamic_Sampler):
    def __init__(self,triples,num_samples,dataset_path,filtered=True):
        model = self.load_rescal(dataset_path)
...
class Adversarial_Sampler(Dynamic_Sampler):
    def __init__(self,triples,num_samples,dataset_path,filtered=True): # Zafar Edit: 22-Oct-2024. Added a parameter 'results_dir' to get path of pre-trained model
        model = self.load_rescal(dataset_path)
